...
def run(seed):
    ...
    for prefix in ['fnc', 'agg', 'im', 'pca', 'dl']:
        train.loc[d21_index, prefix + '_score'] = np.load(folder_preds + '{}_score_seed{}.npy'.format(prefix, seed))
    ...
    # linear
    linear_cols = sorted(list(set(ic_cols + fnc_cols + pca0_cols) - set(['IC_20'])))
    ...
    train_kernel, test_kernel = scale_select_data(train=train, test=test, df_scale=df_scale, cols=kernel_cols, scale_factor=0.2, scale_cols=pca0_cols, sc=StandardScaler())
    ...
    BaggingRegressor(Ridge(alpha=1), n_estimators=100, max_samples=0.2, max_features=0.2, random_state=0)]
    ...
    d21_prediction.to_csv(folder_preds + 'domain2_var1_stack_seed{}.csv'.format(seed), index=False)
    print('domain2_var1 seed pred is saved as', folder_preds + 'domain2_var1_stack_seed{}.csv'.format(seed))
